...
const  ReceivedOrder = ({ order, supplier, closeModal, setReceive ,   getAllReceivedOrders,  getAllPlacedOrders, customer}) => {
  const { toastOptions} = useContext(AuthContext);
  // const { displayImsItems } = useContext(TableContext);
  const receiveItem = async () => {
    try {
      fetch("https://uppist-server.onrender.com/supply/receive-supply-order", {
        method: "PUT",
        headers: {
  ...
  return (
    ...
    >
      <div className="close__modal"  id="bg" onClick={() => setReceive(prevValue => !prevValue)}
><AiOutlineCloseCircle size={25}/></div>
      <h4 style={{ padding: "0", margin: "0" }}>Are you sure this order has been received</h4>
