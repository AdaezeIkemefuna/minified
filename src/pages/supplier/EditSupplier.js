  ...
  const handleSubmit = async () => {
    try {
      const response = await fetch(
        "https://uppist-server.onrender.com/new-supplier",
        {
          method: "POST",
          headers: {
          ...
          <>
                ...
                <form className='supply__order__form' onSubmit={(e) => {e.preventDefault(); handleSubmit()}}>
                 ...
                 <div>
                  ...
                  <input type="text" name="pendingAmount" value={pendingAmount} onChange={(e) => setPendingAmount(e.target.value)}/>
                  </div>
                  </div>
                  
                 <div className='supply__order__form__group2'>
                 <button className='supply__order__form__button1'>Add Supplier</button>
                 <p onClick={() => navigate(-1)} style={{background:"transparent", border:'1px solid var(--primary-color)', color:'var(--primary-color)'}} className='supply__order__form__button1'>Cancel</p>
                 </div>
                 </div>
