...
function SupplyTransaction() {
   ...
   const usedAxios = () => {axios.post(`https://uppist-server.onrender.com/supply/get-total`, {
    supplier: customer,
    })
    ...
    const getTotalPlaceOrders = async (customer) => {
      try {
        const response = await fetch(
          `https://uppist-server.onrender.com/supply/get-total`,
          {
            method: "POST",
            headers: {
 ...
 const getAllPlacedOrders = async (customer) => {
   try {
     const response = await fetch(
       `https://uppist-server.onrender.com/supply/placed-orders`,
       {
         method: "POST",
         headers: {
           "Content-Type": "application/json",
     ...
     const getAllReceivedOrders = async (customer) => {
      try {
        const response = await fetch(
          `https://uppist-server.onrender.com/supply/received-orders`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
 ...
 useEffect(() => {
  ...
  return (
    ...
    <div className="user__details">
      <div className='user__dashboard__header'>
        ...
        onClick={() => {  navigate(`/supplier/${customer}/supplyorder`)} }
          >
          ...
          <div
        className="user__details__order"
        onClick={() => {  navigate(`/supplier/${customer}/receivesupply`)} }
          >
          ...
          <div
        className="user__details__order"
        onClick={() => {  navigate(`/supplier/${customer}/cancelsupply`)} }
          >
            <div  className="order__badge3">
            <span>
              <BsPlusCircle size={20} />
          ...
          <div
        className="user__details__order"
        onClick={() => {  navigate(`/supplier/${customer}/damagedsupply`)} }
          >
            <div  className="order__badge3">
            <span>
              <BsPlusCircle size={20} />
          ...
          <div
        className="user__details__order"
        onClick={() => {  navigate(`/supplier/${customer}/returnsupply`)} }
          >
            <div  className="order__badge3">
            <span>
              <BsPlusCircle size={20} />
    ...
    <div className="user__details__total">
      <p><span className='total__title'>Total : </span>{activePage === "PLACED ORDERS" ? `₦${totalPlacedOrders}` : "₦65,000"}</p>
  ...
  {activePage === "PLACED ORDERS" ? <tbody  className=" row__supplier supply__table">
    
      {placedOrders?.map((order, index) => (
        <tr key={index} className="ims__body">
       <TableRow order={order} index={index}  getAllReceivedOrders={getAllReceivedOrders}
 getAllPlacedOrders={getAllPlacedOrders}  supplier={customer} customer={customer}/>
        </tr>
      ))}
    
  </tbody> : 
    activePage === "RECEIVED ORDERS" ? <tbody  className=" row__supplier supply__table">
...
<tbody  className=" row__supplier supply__table">
  ...
  {products?.map((item, index) => (
    ...
    <td>{item.total}</td>
    <td>{item.date}</td>
<td><span className="ims--cancelled">cancelled</span></td>
    </tr>
  ))}
...
const TableRow = ({order, index, customer, getAllReceivedOrders,getAllPlacedOrders, supplier}) => {
...
return (
   <>
    {receive && (
        ...
        >
          <div>
            <ReceivedOrder customer={customer}   getAllReceivedOrders={getAllReceivedOrders}
 getAllPlacedOrders={getAllPlacedOrders} order={order} supplier={supplier} setReceive={setReceive} closeModal={closeModal} />
          </div>
        </div>
      )}
      ...
      {cancel && (
        ...
        >
          <div>
            <CancelOrder order={order} cancel ={cancel} supplier={supplier} closeModal={closeModal} />
          </div>
        </div>
      )}
