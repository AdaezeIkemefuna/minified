...
function ReceiveSupply() {
  ...
  const handleSubmit = async () => {
    try {
      const response = await fetch(
        "https://uppist-server.onrender.com/supply//receive-supply-order",
        {
          method: "POST",
          headers: {
  ...
  return (
    <div className='supply__order__container form__wrapper'>
       ...
       {currentPage ? (
          <>
                <p className='supply__add'>Add item details to receive an order</p>
                <form className='supply__order__form' onSubmit={(e) => {e.preventDefault()}}>
                 ...
                 <div className='supply__order__form__group'>
                 <button onClick={() => setCurrentPage( prevValue => !prevValue)} className='supply__order__form__button'>Place Order</button>
                 </div>
                 </form>
       </> )  : (
        <>
